...
sub checkBamIndex($) {
    ...
    my $rootfilename = basename($file, ".bam");
    my $dirfilename = dirname($file);

    my $ifile1 = $file . ".bai";
    my $ifile2 = $dirfilename . "/" . $rootfilename . ".bai";
    if((! -f $ifile1) && (! -f $ifile2)) {
       errorX("Can't find index for BAM file '$file'. Checked '$ifile1' and '$ifile2'. ");
    }

    if  ($file =~ /bam.+/ ){
        print STDERR "WARNING: This file will likely fail because its name contains \"bam\" somewhere other than the extension.\n\tFile:\t" . $file . "\n";
        print STDERR "This is a known strelka bug as of vs 1.0.11 and can be worked around by changing the file name so that \"bam\" appears only in the file extension\n";
    }
}
